...
"""
***IDSG模型***
传入source(内部经过Ef,生成r_s)
传入pose shape(内部经过E_l，生成γ)
结合 r_s + γ (内部经过R_l)生成 l_t(即landmark with target pose and source face)
"""
...
读取pose_guide image
"""
pose_path = "./pose_guides/"
pose_list = []
for root, dirs, files in os.walk(pose_path):
    for file in files:
...
"""
读取source image, 每次读取4张图片进行攻击
"""
source_path = './for_demo/'
#读取图片路径保存到列表
source_list = []
...
for i in range(total_batch): #每次读取batch_size=4张图片
    ...
    #读取source图片
    img_crop = cv2.imread(source_list[i*batch_size])
    img_crop_2 = cv2.imread(source_list[i*batch_size+1])
    img_crop_3 = cv2.imread(source_list[i*batch_size+2])
    img_crop_4 = cv2.imread(source_list[i*batch_size+3])
    ...
    for pose_img in pose_list:
        frame = cv2.imread(pose_img)
        ...
        这里传入source_all，需要经过Ef（vggFace2），生成r_s;
        传入shape_all，需要经过E_l（5层fc），γ;
        将r_s和γ 结合（R_l）最后生成l_t
        """
        ...
        # cv2.imwrite('./result/img_shape_fake{}.png'.format(nn), lm_fake*255)#:source1+pose生成的landmark图
